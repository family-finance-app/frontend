...
interface PeriodComparisonStats {
  date: string;
  amount: number;
}

// calculate total income for last three periods
export function getPeriodIncomeComparison(
  ...
  const results: PeriodComparisonStats[] = [];
...
// calculate total expenses for last three periods
...
// calculate total savings balance for last three periods (cumulative balance of all SAVINGS accounts at end of each period)
export function getPeriodSavingsComparison(
  transactions: Transaction[],
  _accounts: Account[] = [],
  period: PeriodType = 'month'
): PeriodComparisonStats[] {
