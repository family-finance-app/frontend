    ...
    <div className="space-y-6">
      {familyGroups && familyGroups.length > 0 && (
        <div className="bg-background-50 dark:bg-background-300 border-l-2 dark:border-l-primary-700 rounded-2xl shadow-financial p-6">
          <h2
            className={`${roboto.className} text-lg font-bold text-primary-800  mb-4`}
          >
          ...
          <div className="space-y-3">
            {familyGroups.map((group) => (
              <div
                key={group.id}
                className="flex items-center justify-between p-3 rounded-lg bg-background-50 dark:bg-background-100 border border-background-100 transition-colors"
              >
